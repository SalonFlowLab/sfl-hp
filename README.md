...
Lucia Eyelash Salon の静的Webサイトです。HTML/CSS/JavaScriptのみで構成し、公開対象のファイルは `public/` に集約しています。
...
ホーム / サロン紹介 / メニュー・料金 / ご予約・来店案内 / スクール / サロン運営の仕組み / Lash Cycle Pro / 同業者向け相談 / アクセス / お問い合わせ / プライバシーポリシー / AI・Lark・公式LINE活用
...
Cloudflare側のプロジェクト名は `lucia-eyelash-salon` を既定値にしています。別案件で使う場合は `package.json` の `config.cloudflare_project_name` と `wrangler.jsonc` の `name` を同じ値に変更してください。
...
- `wrangler.jsonc`: `name`
- `README.md`: サイト名、ページ一覧、Cloudflare Pagesプロジェクト名
- `public/assets/images/logo.jpg` と `public/assets/images/favicon.svg`: ロゴとファビコン
- `public/assets/js/site-chrome.js`: 共通ナビゲーション、ブランド表記、フッター
- `public/pages/`: ページ本文、meta title、meta description
- `public/_headers`: Cloudflare Pages のヘッダー設定
